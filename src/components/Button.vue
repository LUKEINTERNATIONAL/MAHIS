<template>
    <ion-toolbar :color="color" class="_buttons">
        <ion-button v-for="(btn, index) in footerBtns" :key="index" :slot="btn.slot" v-show="btn.visible"
            @click="btn.onClick(btn)" :color="btn.color" :fill="btn.fill" :class="btn.styleClass" :size="btn.size">
