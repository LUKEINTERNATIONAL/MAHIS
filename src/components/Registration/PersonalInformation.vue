...
export default defineComponent({
  ...
  methods: {
    ...
    calculateAge(event: any) {
      const updateGuardianInfo = (value: boolean) =>
        modifyFieldValue(
      ...
      if (event.name == "birthdate") {
        HisDate.getAgeInYears(event.value) < 17
          ? updateGuardianInfo(false)
          : updateGuardianInfo(true);

        modifyFieldValue(
          this.personInformation,
          "estimation",
          "value",
          HisDate.getAgeInYears(event.value)
        );
    ...
    calculateDoB(event: any) {
      if (event.name == "estimation") {
        ...
        modifyFieldValue(
          ...
          "birthdate",
          "value",
          HisDate.toStandardHisDisplayFormat(`${year}-06-15`)
        );
        return `${year}-06-15`;
      }
    },
