    ...
    <table class="report-table" :class="config?.tableCssTheme || ''">
        ...
        <tbody v-if="!noData">
            <tr v-for="(row, rowIndex) in activeRows" :key="rowIndex">
                <td v-for="(item, itemIndex) in row" :key="itemIndex" :colspan="item.colspan || 0" :class="item.cssClass" :style="item.style">
                    <div v-if="item.event">
                        <a
                            href="#"
                            class="his-sm-text"
                        ...
                        <ion-button
                            ...
                            :class="item.cssClass"
                            :style="item.style"
                            :disabled="item?.event?.disabled != undefined ? item.event.disabled === true : false"
                    ...
                    <div v-else>
                        <ion-icon class="his-sm-text" slot="start" aria-hidden="true" :icon="item.td"></ion-icon>
                    </div>
                </td>
            </tr>
        </tbody>
...
import { ColumnInterface, RowInterface, TableInterface } from "@/components/DataViews/tables/ReportDataTable";
    ...
    methods: {
        ...
        searchDataSet(searchTerm: string, dataset: Array<any>) {
            return dataset.filter((r: any) => {
                const rowText = JSON.stringify(r)
                    .match(/"td":"(.*?)"/g)
                    ?.join(" ")
                    .replace(/"td":|"/g, "");
