...
import { useAllegyStore, searchHealthcareEquipmentAllergies, concatenateArrays } from "../../../OPD/stores/AllergyStore"
import { ConceptService } from "@/services/concept_service"
import { ref, watch, computed, onMounted, onUpdated } from "vue"
import { ConceptName } from "@/interfaces/conceptName";

const store = useAllegyStore()
const selectedAllergiesList = computed(() => store.selectedAllergiesList)
const input = ref()
const drugName = ref("")
const allergiesList = computed(() => store.allergiesList)
...
function selectAl(item: any) {
    ...
    AllergyStore.setAllAllergiesList(item)
...
function saveStateValuesState() {
    const allergyStore = store
    allergyStore.setAllergiesList(allergiesList);
}
...
async function FindAllegicDrugName(text: any) {
    ...
    allergyStore.setAllergiesList(temp_data_2)
}
</script>
