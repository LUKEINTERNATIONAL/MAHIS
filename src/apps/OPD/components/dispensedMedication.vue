<template>
  <ion-list>
    <ion-label
      >Allergies (Medication, Healthcare items, Environment and Food)</ion-label
    >
    <ion-row>
    ...
    <div
      v-if="
        dispensationStore.getDrugPrescriptions() &&
        dispensationStore.getDrugPrescriptions().length > 0
      "
    >
      <dynamic-list
    ...
    <div v-else>
      Loading, Please Wait. If this takes more than 2 seconds then something
      went wrong...
    </div>
    <div>
      <div class="space3" />
...
export default defineComponent({
  watch: {
    async demographics(newValue, oldValue) {
      this.setListData(await this.prepareData(newValue.patient_id));
  ...
  async mounted() {
    ...
    this.setListData(await this.prepareData(this.demographics.patient_id));
  },
  methods: {
    ...
    async prepareData(patientId: any) {
      ...
      return !isEmpty(obs)
        ? Promise.all(
            obs.map(async (ob: any) => {
              return {
                name: await ObservationService.getConceptName(
                  ob["value_coded"]
                ),
...
const selectedAllergiesList2 = computed(
  () => store2.selectedMedicalAllergiesList
);
...
async function fetchMedication(){
  ...
  const { previousDrugPrescriptions } =
    await previousTreatment.getPatientEncounters();

  dispensationStore.setDrugPrescriptions(
    previousDrugPrescriptions[0]?.previousPrescriptions
  );

  for (
    let index = 0;
    index < dispensationStore?.getDrugPrescriptions()?.length;
    index++
  ) {
...
function saveDispensations() {
  ...
  const dispensation_srvc = new DispensationService(
    demographics.value.patient_id,
    Service.getUserID() as any
  );
  const dispensation_payload: any =
    dispensationStore.getDispensedMedicationsPayload();
