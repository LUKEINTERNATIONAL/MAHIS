<template>
    ...
    <table class="custom-table">
        ...
        <thead>
            <tr>
                <th></th>
                <th colspan="2" @click="selectSection('fixed')" :class="{ selected: selectedSection === 'fixed' }">Static</th>
                <th colspan="2" @click="selectSection('outreach')" :class="{ selected: selectedSection === 'outreach' }">Outreach</th>
            </tr>
            <tr>
                <th></th>
                <th @click="selectColumn('fixedLessThan1y')" :class="{ selected: selectedColumn === 'fixedLessThan1y' }">&lt;1y</th>
                <th @click="selectColumn('fixedMoreThan1y')" :class="{ selected: selectedColumn === 'fixedMoreThan1y' }">&gt;1y</th>
                <th @click="selectColumn('outreachLessThan1y')" :class="{ selected: selectedColumn === 'outreachLessThan1y' }">&lt;1y</th>
                <th @click="selectColumn('outreachMoreThan1y')" :class="{ selected: selectedColumn === 'outreachMoreThan1y' }">&gt;1y</th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="(row, index) in tableData" :key="index">
                ...
                >
                    <input v-model="row.outreach.lessThan1y" class="editable-input" />
                </td>
                <td
                    :class="{
                ...
                >
                    <input v-model="row.outreach.moreThan1y" class="editable-input" />
                </td>
            </tr>
        </tbody>
    </table>
...
export default defineComponent({
    ...
    watch: {
        $route: {
            async handler(data) {
                if (data.name == "EIRReport") await this.getDrugs();
    ...
    methods: {
        ...
        async initReport() {
            ...
            data.less_than_one_year.forEach((item: any) => {
                this.fillCells(item, "lessThan1y");
            });
            data.greater_than_one_year.forEach((item: any) => {
        ...
        async openPatientProfile(client_id: any) {
            ...
            patientData.patient_identifiers.forEach(async (indnt: any) => {
                if (indnt.identifier_type == 3) {
                    ...
                    this.setServerRecord(patientData2[0]);
