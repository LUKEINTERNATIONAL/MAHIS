<template>
    <div class="container">
        <ion-card class="section">
            <ion-card class="section">
                <ion-card-header>
                    <ion-card-title class="dashed_bottom_border sub_item_header"
                        >The client is below 19, complete the HEADSS assessment</ion-card-title
                    ></ion-card-header
                >
                <ion-card-content>
...
export default defineComponent({
    ...
    computed: {
        ...
        Abortions() {
            return getFieldValue(this.prevPregnancies, "Abortions", "value");
        },
    },
    created() {
    ...
    methods: {
        ...
        validaterowData(event: any) {
            ...
            const abortionsField = this.prevPregnancies.find(
                (field: any) => field.data.rowData.length > 1 && field.data.rowData[1].colData[0].name === "Abortions/Miscarriages"
            );

            const stillBirthsField = this.prevPregnancies.find(
                (field: any) => field.data.rowData.length > 1 && field.data.rowData[1].colData[1].name === "Stillbirths"
            );
