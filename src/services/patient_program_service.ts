...
export class PatientProgramService extends ProgramService {
    ...
    async getProgram() {
         ...
         */
        const programs = req
            .filter((p: any) => p.program.program_id === ProgramService.getProgramID())
            .map((p: any) => {
                /**
        ...
        return !isEmpty(programs) ? programs[0] : defaults;
    }

    getProgramStates() {
        return ProgramService.getPatientStates(this.patientId, this.programId);
    }
    getProgramCurrentStates() {
        return ProgramService.getPatientCurrentStates(this.patientId, this.programId);
    }
