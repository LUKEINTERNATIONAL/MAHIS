...
export function modifyAlertsValue(data: any, alertName: any, element: any, newValue: any) {
    for (const item of data) {
        const alertData = data.flatMap((item: any) => item.alerts).find((alert: any) => alert?.name === alertName);
...
export async function validateStore(store: any, schema: any, nameValues: ExtractedNameValue[]): Promise<boolean> {
    ...
    const formObject = nameValues.reduce((acc: any, current: ExtractedNameValue) => {
        if (current.name) {
