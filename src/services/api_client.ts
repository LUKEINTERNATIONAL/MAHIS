...
const ApiClient = (() => {
    ...
    async function execFetch(uri: string, params: any) {
        // Execute fetch with provided URI and parameters
        params = { ...params, mode: 'cors' };
        
        try {
            // Emit event before making API request
            EventBus.emit(ApiBusEvents.BEFORE_API_REQUEST, uri);

            // Make API request
            const response = await instance(uri, params);

            // Emit event after API request completes
            EventBus.emit(ApiBusEvents.AFTER_API_REQUEST, response);
            ...
            // Emit event if API request fails
            if (error.toString().match(/NetworkError|Failed to fetch/i)) {
                EventBus.emit(ApiBusEvents.ON_API_CRASH, error);
            } else {
                EventBus.emit(ApiBusEvents.AFTER_API_REQUEST, {});
            }
    ...
    return { get, post, put, remove, healthCheck };
})();
