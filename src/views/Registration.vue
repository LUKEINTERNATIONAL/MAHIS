<template>
  <ion-page>
    ...
    <ion-content>
      ...
      <div
        v-if="registrationType == 'manual' && registrationDisplayType == 'grid'"
      >
      ...
      <div
        v-if="registrationType == 'manual' && registrationDisplayType == 'list'"
      >
  ...
  methods: {
    ...
    async createPatient() {
      ...
      if (await this.validations(this.personInformation, fields)) {
        ...
        if (Object.keys(this.personInformation[0].selectedData).length === 0)
          return;
        const registration: any = new PatientRegistrationService();
        await registration.registerPatient(
        ...
        if (
          Object.keys(this.guardianInformation[0].selectedData).length === 0
        ) {
          if (
    ...
    openNewPage(url: any, item: any) {
      ...
      demographicsStore.setDemographics({
        name:
          item.person.names[0].given_name +
          " " +
          item.person.names[0].family_name,
    ...
    buildPersonalInformation() {
      this.personInformation[0].selectedData = {
        ...
        home_region: getFieldValue(
          this.personInformation,
          "homeDistrict",
          "value"
        ),
        home_district: getFieldValue(
          this.personInformation,
          "homeDistrict",
          "value"
        ),
        ...
        home_village: getFieldValue(
          this.personInformation,
          "homeVillage",
          "value"
        ),
        current_region: getFieldValue(
          this.personInformation,
          "currentDistrict",
          "value"
        ),
        current_district: getFieldValue(
          this.personInformation,
          "currentDistrict",
          "value"
        ),
